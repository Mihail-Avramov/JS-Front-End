...
describe("E2E tests", function () {
    ...
    describe("Sprint Board Tests", () => {
        it("Load Board (loads all in correct categories)", async () => {
            ...
            const [todoTask] = await page.$$eval(`#todo-section li`, (t) => t.map((s) => s.textContent));
            const [inProgressTask] = await page.$$eval(`#in-progress-section li`, (t) => t.map((s) => s.textContent));
            const [codeReviewTask] = await page.$$eval(`#code-review-section li`, (t) => t.map((s) => s.textContent));
            const [doneTask] = await page.$$eval(`#done-section li`, (t) => t.map((s) => s.textContent));
        ...
        it("Create Task (successful create & clear inputs)", async () => {
            ...
            const [request] = await Promise.all([onRequest(), page.click("#create-task-btn")]);

            const postData = JSON.parse(request.postData());

            const [titleValue] = await page.$$eval(`#title`, (t) => t.map((s) => s.value));
            const [descriptionValue] = await page.$$eval(`#description`, (t) => t.map((s) => s.value));
        ...
        it("Move Task (from ToDo to In Progress)", async () => {
            ...
            const [request] = await Promise.all([onRequest(), page.click("#todo-section li button")]);
        ...
        it("Move Task (from In Progress to Code Review)", async () => {
            ...
            const [request] = await Promise.all([onRequest(), page.click("#in-progress-section li button")]);
        ...
        it("Move Task (from Code Review to Done)", async () => {
            ...
            const [request] = await Promise.all([onRequest(), page.click("#code-review-section li button")]);
